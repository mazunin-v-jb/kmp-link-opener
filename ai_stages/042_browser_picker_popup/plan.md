...
## Implementation notes (что добавилось после первоначального merge)

Базовая стадия 4.2 закрыта в `d3fdd05 Stage/04.2 browser picker popup (#4)`. Полировка UX добавлена в `1e04325 Bugfix/picker popup polish (#6)`. Что есть на main сверх первоначального acceptance criteria:

- **Drag handle.** Хедер picker'а (URL-блок) обёрнут в `androidx.compose.foundation.window.WindowDraggableArea`, так что popup можно таскать за этот блок. `BrowserPickerScreen` принимает `headerWrapper: @Composable (content: @Composable () -> Unit) -> Unit` (default = passthrough); `PickerWindow` подсовывает `WindowDraggableArea`-вариант. Слот используется только потому, что `WindowDraggableArea` — расширение `WindowScope`, и его легче дернуть из `:desktopApp/jvmMain` (внутри `Window {}` блока), чем тащить в `commonMain`.
- **Scrollable expanded list.** При клике «Show all» popup растёт с 320×320 до 320×480, и список оборачивается в `verticalScroll`-Column с heightIn cap'ом в 6 рядов плюс `VerticalScrollbar`. Реализовано через дополнительный `onExpand: () -> Unit = {}` callback в `BrowserPickerScreen` — `PickerWindow` ловит его и пишет `windowState.size = DpSize(320.dp, 480.dp)`. `WindowState.size` реактивен, поэтому одного присваивания хватает.
- **Dismiss on focus loss** уже был в первоначальном merge — не сломан полировкой.

TD-1 (overlay над fullscreen на macOS) — **по-прежнему не решён**. Хелпер `MacOsAlwaysOnTopOverFullScreen` оставлен в коде с подробным doc-блоком о провалившихся попытках; см. секцию TD-1 выше + общую запись в `TECHDEBT.md`.
