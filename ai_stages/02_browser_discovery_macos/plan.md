...
На реальной машине разработчика в выдачу также попадают приложения, которые регистрируют http/https для специфических сценариев (VLC — для streaming-URL, MKPlayer — без чёткого назначения). На стадии 2 это сознательно не отфильтровывается: критерий «зарегистрирован как handler для http/https» — это и есть наш контракт «может открыть веб-ссылку». Если VLC в системе ассоциирован с http (что macOS позволяет пользователю сделать вручную), он действительно может открывать ссылки, и пользователь, выбравший его, получает ожидаемое поведение. UI исключений из стадии 4 даст возможность скрывать такие записи.
